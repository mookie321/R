...
system.time(for(i in 1:(max.order+1)){
  for(j in 1:(max.order+1)){
    currentArima <- arima(x,order=c(i-1,0,j-1))
    aic.matrix1[i,j] <- AIC(currentArima) 
...
inds <- arrayInd(which.min(AIC.matrix), dim(AIC.matrix))
aic.matrix1<-create.matrix(nrow = max.order+1,ncol= max.order+1)
